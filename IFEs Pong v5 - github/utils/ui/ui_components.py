...
import pygame
import time
from utils.game.game_constants import SPECIAL_KEYS
from utils.ui.ui_constants import (X_SETTINGS_SPACE_OFFSET, BORDER_RAD, TV_INPUT_DELAY_TIMER,
                                   TV_INPUT_MAX_WAIT_DELAY_TIMER, BORDER_RAD_BUTTON, BLINK_TIMER,
                                   FN3_SIZE, FOCUS_RECT_WIDTH, CURSOR_ADJUST
                                   )
import utils.helper_functions as mods

# I will fix the code later, lemme just say that IT IS BAD
class Input:
    def __init__(self, screen: pygame.Surface, init_input: str, max_tv_len: int, pos: tuple, font: pygame.font.Font, fg_color, border_radius: int = BORDER_RAD, op_space_offset: int = X_SETTINGS_SPACE_OFFSET, f_r_width: int = FOCUS_RECT_WIDTH, width: int = None) -> None:
...
class Button:
    ...
    def on_activate(self, mouse_rect, mouse_was_clicked, action_performed, action_on_hover, action_on_not_click, sound_info: tuple, bounce_delay: int):
        if self.txt is not None:
            self.txt_surf = pygame.transform.scale(pygame.font.Font(size=int(self.size) if isinstance(self.size, int) or isinstance(self.size, float) else int(self.size[1])).render(self.txt, False, self.txt_color), (self.size, self.size) if isinstance(self.size, int) or isinstance(self.size, float) else self.size)
    ...
    def activate_button(self, mouse_rect: pygame.Rect, mouse_clicked: bool, on_click=None, on_hover=None, on_not_click=None, pos=None, color: tuple | Any = None, bounce_delay=20, sound_info: tuple = None, text: str = None):
        if text is not None:
            self.txt = text
            self.txt_surf = pygame.transform.scale(pygame.font.Font(size=int(self.size) if isinstance(self.size, int) or isinstance(self.size, float) else int(self.size[1])).render(self.txt, False, self.txt_color), (self.size, self.size) if isinstance(self.size, int) or isinstance(self.size, float) else self.size)
...
class Selector:
    ...
    def display_outline(self, color):
        self.outline = pygame.Rect(self.outline_rect.x - X_SETTINGS_SPACE_OFFSET//2,
                                   self.outline_rect.y,
                                   (self.outline_surf.get_width() + X_SETTINGS_SPACE_OFFSET) if self.width is None else self.width,
...
class ClickableText:
    ...
    def _check_clicked(self, mouse_rect: pygame.Rect, clicked: bool, func):
    ...
    def _draw(self, pos: Union[tuple, list]):
        dest_size = self.text_surf.get_size()[0] + self.hover_bg_offset, self.text_surf.get_size()[1] + self.hover_bg_offset
        self.hover_rect.topleft = self.text_rect.center[0] - (dest_size[0] / 2), self.text_rect.center[1] - (dest_size[1] / 2)
        if pos is not None:
